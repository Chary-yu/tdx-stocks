...
def stock_display(row_or_symbol: object, stock_names: dict[StockKey, str] | None = None, market: object | None = None) -> str:
    ...
    if isinstance(row_or_symbol, dict):
        ...
        if key is not None:
            return stock_names.get(key) or display_code(*key)
        return str(row.get("display_symbol") or row.get("symbol") or "无")
