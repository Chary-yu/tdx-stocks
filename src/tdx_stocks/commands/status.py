...
def _build_status_payload(config: AppConfig, *, config_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "latest_run_report": (data_root / "reports" / "latest.json").as_posix(),
        "latest_daily_report": (data_root / "reports" / "daily" / "latest.json").as_posix() if latest_daily else None,
        "latest_portfolio_report": (data_root / "reports" / "portfolios" / "latest.json").as_posix() if latest_portfolio else None,
